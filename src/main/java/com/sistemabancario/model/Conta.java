...
public class Conta implements Cadastro {
    ...
    public void depositoDinheiro(final double valor) {
        if (valor <= 0) {
            throw new IllegalArgumentException("Numero inserido é inválido.");
        } else {
    ...
    public void setNumero(String numero) {
        if (!numero.matches("\\d{5}-\\d") || numero.equals(null)) {
    ...
    public void setLimite(double limite) {
        if (!especial && limite > 0) {
            throw new IllegalStateException("Somente conta especial tem limite.");
        }
        this.limite = limite;
    }
}
